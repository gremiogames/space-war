...
function uniqueShipIds(shipIds) {
    return Array.from(new Set(shipIds.filter((shipId) => SHIPS_BY_ID.has(shipId))));
}
...
function openStoreModal(scene) {
    ...
    const refresh = () => {
        ...
        rows.forEach((row) => {
            ...
            if (isOwned) {
                row.actionButton
                    .setFillStyle(0x1d3448, 1)
                    .setStrokeStyle(2, 0x37688f);
                row.actionLabel.setText("EQUIPAR").setColor("#d7ebff");
                row.actionType = "equip";
            } else {
                row.actionButton
                    .setFillStyle(0x302e1c, 1)
                    .setStrokeStyle(2, 0x766a2f);
                row.actionLabel
